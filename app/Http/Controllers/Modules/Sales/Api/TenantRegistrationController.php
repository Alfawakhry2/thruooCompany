...
class TenantRegistrationController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        // Track what we create so we can manually roll back on failure
        $tenant = null;
        $company = null;

        try {
            // ====================================================================
            // STEP 1: Create Tenant (Owner) — runs on landlord (mysql)
            // ====================================================================
            Log::info('Creating Tenant (Owner)...');
            ...
            // ====================================================================
            // STEP 2: Create Company — runs on landlord (mysql)
            // ====================================================================
            Log::info('Creating Company...');
            ...
            // ====================================================================
            // STEP 3: Create Company Details — runs on landlord (mysql)
            // ====================================================================
            Log::info('Creating Company Details...');
            ...
            // ====================================================================
            // STEP 6: Seed Roles and Permissions — tenant DB
            // ====================================================================
            Log::info('Seeding roles and permissions...');
            $company->makeCurrent();
            ...
            // ====================================================================
            // STEP 7: Create Module Records — tenant DB
            // ====================================================================
            Log::info('Creating module records...');
            $company->makeCurrent();
            ...
            // ====================================================================
            // STEP 7.5: Create Default Branch — tenant DB (already current)
            // ====================================================================
            Log::info('Creating default branch...');
            ...
            // ====================================================================
            // STEP 8: Create Admin User — tenant DB (already current)
            // ====================================================================
            Log::info('Creating admin user...');
            ...
            // ====================================================================
            // STEP 9: Assign Admin Role — tenant DB (already current)
            // ====================================================================
            Log::info('Assigning Admin role to owner...');
            ...
            // ====================================================================
            // STEP 9.5: Assign Admin User to Default Branch — tenant DB
            // ====================================================================
            Log::info('Assigning admin user to default branch...');
            $adminUser->branches()->attach($defaultBranch->id);
            Log::info("Admin user assigned to branch: {$defaultBranch->name}");
            ...
            // ====================================================================
            // STEP 10: Create Team Members — tenant DB (already current)
            // ====================================================================
            if ($request->has('team_members') && is_array($request->team_members)) {
                Log::info('Creating team member users...');
            ...
            // ====================================================================
            // STEP 11: Generate token — tenant DB (already current)
            // ====================================================================
            $token = $adminUser->createToken('auth-token')->plainTextToken;
            ...
            // ====================================================================
            // MANUAL ROLLBACK — delete what we created on landlord DB
            // ====================================================================
            if ($company) {
                try {
                    ...
                    Log::error('Cleanup error (database): ' . $cleanupError->getMessage());
                }

                try {
                    // 2. Delete CompanyDetails
                    CompanyDetails::where('company_id', $company->id)->delete();
                    Log::info('CompanyDetails cleanup completed');
                } catch (\Exception $cleanupError) {
                    Log::error('Cleanup error (details): ' . $cleanupError->getMessage());
                }

                try {
                    // 3. Delete Company
                    $company->delete();
                    Log::info('Company cleanup completed');
                } catch (\Exception $cleanupError) {
                    Log::error('Cleanup error (company): ' . $cleanupError->getMessage());
                }
            }
            ...
            if ($tenant) {
                try {
                    ...
                    Log::error('Cleanup error (tenant): ' . $cleanupError->getMessage());
                }
            }
